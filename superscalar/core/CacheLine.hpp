...
public:
    ...
    static_assert(
        CPUAddressWidth >= OffsetBits + IndexBits,
        "Address width is too small for the cache configuration."
    );


    static constexpr std::size_t TagBits =
        CPUAddressWidth - OffsetBits - IndexBits;
    ...
    /*
     Installing a complete cache line using
    */
    void install_line(
        std::size_t address,
        const std::array<
    ...
    /*
      refilling a specific cache line

     the controller can use this when it has
     already calculated the index and tag
     */
    void refill(
    ...
    [[nodiscard]]
    static std::size_t get_line_index(
        std::size_t address
    ) noexcept
    {
    ...
    [[nodiscard]]
    static std::size_t get_instruction_offset(
        std::size_t address
    ) noexcept
    {
...
private:
    ...
    [[nodiscard]]
    static std::size_t get_address_value(
    ...
    {
        const std::size_t address_value =
            get_address_value(address);

        const std::size_t line_index =
            get_line_index(address_value);

        const std::size_t tag =
            get_tag(address_value);

        const std::size_t instruction_offset =
            get_instruction_offset(address_value);
        ...
        if (cache_hit)
        {
            write_output(
                instruction,
                line.instructions[instruction_offset]
            );
        }
        else
        {
